...
namespace Script
{
    ...
    public class MapController : MonoBehaviour
    {
        ...
        private readonly List<Vector2> posList = new List<Vector2>();

        private static readonly List<Vector2> SuperWallPosList = new List<Vector2>();
        ...
        private static void DestroyMap()
        {
            ...
            var swalls = GameObject.FindGameObjectsWithTag("SuperWall");
            var boo = GameObject.FindGameObjectsWithTag("Boom");
            var boe = GameObject.FindGameObjectsWithTag("Bomb");
            var props = GameObject.FindGameObjectsWithTag("Props");
            var door = GameObject.FindGameObjectWithTag("Door");
            var doorwall = GameObject.FindGameObjectWithTag("DoorWall");
        ...
        public static bool IsSuperWallPos(Vector2 ve)
        {
            ...
            {
                if (Math.Abs(v.x - ve.x) < 0.1f && Math.Abs(v.y - ve.y) < 0.1f)
                {
                    s = true;
                    break;
                }
